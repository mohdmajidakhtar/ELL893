(function(){function r(e,n,t){function o(i,f){if(!n[i]){if(!e[i]){var c="function"==typeof require&&require;if(!f&&c)return c(i,!0);if(u)return u(i,!0);var a=new Error("Cannot find module '"+i+"'");throw a.code="MODULE_NOT_FOUND",a}var p=n[i]={exports:{}};e[i][0].call(p.exports,function(r){var n=e[i][1][r];return o(n||r)},p,p.exports,r,e,n,t)}return n[i].exports}for(var u="function"==typeof require&&require,i=0;i<t.length;i++)o(t[i]);return o}return r})()({1:[function(require,module,exports){
    ...
    window.mqttClientMessageHandler = function(topic, payload) {
       ...
       if(receivedMessage.device == "smart_bulb1")
       {
         ...
         {
            if(receivedMessage.params.color != "#ffffff")
            {
               document.getElementById('livingroom').style.opacity = 0.6;
               document.getElementById('triangle-bottomright').style.opacity = 0.6;
               document.getElementById('livingroomlamp').style.opacity = 0.7;
               ...
               var messageTopublish = {response: "Smart_bulb1 Color Changed"};
               var finalMessage = JSON.stringify(messageTopublish)
               ...
               var publishTopic = "ELL893/muneeb_majid/smarthome/mqtt/smart_bulb1";
               var messageTopublish = {response: "Smart_bulb1 Turned On in living room"};
               var finalMessage = JSON.stringify(messageTopublish)
               mqttClient.publish(publishTopic, finalMessage);
            }
         }else
         {
            ...
            var publishTopic = "ELL893/muneeb_majid/smarthome/mqtt/smart_bulb1";
            var messageTopublish = {response: "Smart_bulb1 Turned Off in living room"};
            var finalMessage = JSON.stringify(messageTopublish)
            mqttClient.publish(publishTopic, finalMessage);
         }
      }else if (receivedMessage.device == "smart_ac1")   // Code to check if device is smart_ac1
      {
         if(receivedMessage.params.power == true && receivedMessage.params.h_direction == "rotate(-45deg)")
         {
            var old_temperature = document.getElementById('temperature').innerText;
            if(old_temperature == receivedMessage.params.temperature)
            {
               ...
               var messageTopublish = {response: "Smart_ac1 is On in bedroom and Wind vents are in Left direction"};
               var finalMessage = JSON.stringify(messageTopublish)
               mqttClient.publish(publishTopic, finalMessage);
            }
            else{
               ...
               document.getElementById('temperature').textContent = receivedMessage.params.temperature;
               var publishTopic = "ELL893/muneeb_majid/smarthome/mqtt/smart_ac1";
               var messageTopublish = {response: "Smart_ac1 is On in bedroom and Temperature changed"};
               var finalMessage = JSON.stringify(messageTopublish)
               mqttClient.publish(publishTopic, finalMessage);
            }
         }
         else if(receivedMessage.params.power == true &&  receivedMessage.params.h_direction == "rotate(0deg)")
         {
            var old_temperature = document.getElementById('temperature').innerText;
            if(old_temperature == receivedMessage.params.temperature)
            {
               ...
               document.getElementById('temperature').textContent = receivedMessage.params.temperature;
               var publishTopic = "ELL893/muneeb_majid/smarthome/mqtt/smart_ac1";
               var messageTopublish = {response: "Smart_ac1 is On in bedroom and Wind vents are in Center direction"};
               var finalMessage = JSON.stringify(messageTopublish)
               mqttClient.publish(publishTopic, finalMessage);
            }
            else
         ...
         else if(receivedMessage.params.power == true &&  receivedMessage.params.h_direction == "rotate(45deg)")
         {
            var old_temperature = document.getElementById('temperature').innerText;
            if(old_temperature == receivedMessage.params.temperature)
            {
               ...
               document.getElementById('temperature').textContent = receivedMessage.params.temperature;
               var publishTopic = "ELL893/muneeb_majid/smarthome/mqtt/smart_ac1";
               var messageTopublish = {response: "Smart_ac1 is On in bedroom and Wind vents are in Right direction"};
               var finalMessage = JSON.stringify(messageTopublish)
               mqttClient.publish(publishTopic, finalMessage);
            }
            else
            ...
            var publishTopic = "ELL893/muneeb_majid/smarthome/mqtt/smart_ac1";
            var messageTopublish = {response: "Smart_ac1 Turned Off in bedroom"};
            var finalMessage = JSON.stringify(messageTopublish)
            mqttClient.publish(publishTopic, finalMessage);
         }
      }
      else if (receivedMessage.device == "smart_fan1")  // Code to control if device is smart_fan1
      {
         if(receivedMessage.params.power == true)
         {
            ...
            var messageTopublish = {response: "Fan turned On in living room"};
            var finalMessage = JSON.stringify(messageTopublish)
            mqttClient.publish(publishTopic, finalMessage);
         }
         else
         {
            ...
            var publishTopic = "ELL893/muneeb_majid/smarthome/mqtt/smart_fan1";
            var messageTopublish = {response: "Fan turned Off in living room"};
            var finalMessage = JSON.stringify(messageTopublish)
            mqttClient.publish(publishTopic, finalMessage);
         }
      }
      else if (receivedMessage.device == "smart_lock1")  // Code to control if device is smart_lock1
      {
         if(receivedMessage.params.door_status == "unlocked")
         {
            ...
            var messageTopublish = {response: "Door Unlocked in living room"};
            var finalMessage = JSON.stringify(messageTopublish)
            mqttClient.publish(publishTopic, finalMessage);
         }else{
            ...
            var publishTopic = "ELL893/muneeb_majid/smarthome/mqtt/smart_lock1";
            var messageTopublish = {response: "Door Locked in living room"};
            var finalMessage = JSON.stringify(messageTopublish)
            mqttClient.publish(publishTopic, finalMessage);
         }
      }
       messageHistory = messageHistory + "[Time: " + getTime() + '] ' + "[Message: "+ payload.toString() + "]" + '</br>';
    ...
    //
    // Handle the UI to update the data we're publishing (Not used from Emulator side)
    //
